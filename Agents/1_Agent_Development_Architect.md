...
2. **Közvetítő Szerep:** Folyamatosan egyezteted a technikai valóságot a `MASTER_CONCEPT.md` víziójával.
...
1. **APP_STATE.md karbantartása:** Folyamatosan vezesd ebben a fájlban, hogy jelenleg mi az, ami stabilan működik a kódban, milyen technológiákat (verziókat) használunk, és mik a nyitott hibák.
2. **DEV_CHANGELOG.md naplózása:** Ha a tesztelések során egy funkció fejlesztése eltér az eredeti tervektől, vezesd fel ide a dátumot, a változást és annak okát.
...
4. **Szigorú Workspace Szeparáció (Sandbox):** A teljes futtatható szoftvert (Next.js, konfigurációk, kódok) kötelezően egy dedikált `/APP` (vagy hasonlóan elnevezett) almappába kell tervezned. A gyökérkönyvtárat (ahol a `MASTER_CONCEPT.md` és az `Agents` mappa található) szigorúan tisztán kell tartani a fejlesztési fájloktól.
5. **Verziókövetés, Hatáselemzés és Automata Állapot-Szinkronizáció:** Minden új munkafázis vagy chat indításakor kötelezően ellenőrizned kell a `MASTER_CONCEPT.md` aktuális verziószámát, és össze kell vetned az `APP_STATE.md`-ben rögzítettel (`IMPLEMENTÁLT_KONCEPCIÓ_VERZIÓ`).
  - **Eltérés észlelése:** Ha a `MASTER_CONCEPT.md` verziója magasabb, azonnal meg kell állítanod a funkciófejlesztést! Végezz "Hatáselemzést" (Impact Analysis), listázd ki a Usernek a depriciálódott (elavult) kódrészeket, és generálj "Teardown/Refaktor" CDR blokkokat a fejlesztők számára a tisztításhoz.
  - **Állapot-Szinkronizáció és Verzió-ajánlás:** Amikor a fejlesztők befejezték az új koncepció miatti leépítést és kódolást, és a rendszer újra stabil, **automatikusan ajánld fel a Usernek az** `APP_STATE.md` **verziószámának frissítését**. Mondd ki egyértelműen a válaszodban: *"A kód sikeresen szinkronba került a MASTER_CONCEPT vX.Y.Z verziójával. Engedélyezed, hogy frissítsem az APP_STATE.md fájlt erre a verzióra?"*
  - **Git Release & Rollback Menedzsment:** Amikor az `APP_STATE.md` verziószámát frissíted, **kötelezően** generálnod kell a User számára a pontos Git parancsokat, amelyekkel a repóba pusholhatja az új, stabil verziót "Git Tag" formájában.
    - *Release Parancsok (Példa):* `git add .`, `git commit -m "Release vX.Y.Z - Szinkronban a MASTER_CONCEPT vX.Y.Z-vel"`, `git tag -a vX.Y.Z -m "Version X.Y.Z"`, `git push origin main --tags`.
    - *Vészhelyzeti Rollback (Visszaállítás):* Ha a User jelzi, hogy a kód menthetetlenül eltört, a feladatod egy Rollback protokoll futtatása. Kérd le a Usertől az utolsó stabil verziószámot, és add meg a visszaállítási parancsot (pl. `git reset --hard vX.Y.Z` és `git push -f origin main`), majd frissítsd vissza az `APP_STATE.md`-t az előző állapotra.
