...
Dátum: 2026-06-04
Kezdeti állapot: A MASTER_CONCEPT.md beolvasása és kibővítése a fogalom-evolúciós, lexikalizációs, szabály-hierarchia és ontológiai átrendezési szabályokkal (10. fejezet).
...
- **Fázisok és elért mérföldkövek:**
  ...
  5. **Fogalom-Evolúció & Ontológiai Átrendezések (Log_005):** Kidolgoztuk a kombinált fogalmak alapszóvá minősítésének szabályait, a 2hxR/2hxL szintaxist, a hanyag evolúciós defaults kezelését, a Szabály-Hierarchiát, a Szeparációs Állapotgépet, valamint a **Dilemma 5s->5a (OntoShift)** mentén az ontológiai átrendezések és a szemantikai elcsúszások HITL-alapú irányított migrációját.
- **Rendszerszintű Igazságok (MASTER_CONCEPT-be rögzítve):**
  ...
  - **OntoShift Pipeline:** Ha egy régi bázisfogalomról kiderül, hogy az egy új, absztraktabb fogalom gyermeke (pl. `5s` -> `5a`), az `5s` nem törlődik (fizikailag érvényes marad a kezdő órákon), de a `parent_term_id` beállítódik az `5a`-ra. A rendszer felajánlja az irányított curriculum migrációt (HITL panel: Keep Specific, Generalize, Deconstruct).
...
- **Tervezés és Megoldások:**
  - **MASTER_CONCEPT Frissítés:** Bevezettük a **10. E. Komponens-Módosulási Konfliktusok és Szabály-Hierarchiát**, valamint a **10. F. Ontológiai Átrendezés és Szemantikai Elcsúszást** (az `5s` -> `5a` példával demonstrálva).
  - **Szoftveres Implementáció:** Megterveztük a Szeparációs Állapotgépet, a prioritás-alapú döntési mátrixot, a `term_components` kapcsolótáblán futó **OntoShift Pipeline**-t és a HITL alapú, irányított curriculum migrációs felületet.
